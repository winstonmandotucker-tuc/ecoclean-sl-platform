...
export async function audioApi(path:string,payload:unknown){const response=await fetch(`${API_URL}${path}`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!response.ok){const body=await response.json().catch(()=>({}));throw new ApiError(body.error||'Audio request failed.',response.status);}return response.blob();}
